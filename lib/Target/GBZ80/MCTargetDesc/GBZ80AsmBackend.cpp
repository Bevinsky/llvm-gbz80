//===-- GBZ80AsmBackend.cpp - GBZ80 Asm Backend  ------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the GBZ80AsmBackend class.
//
//===----------------------------------------------------------------------===//
...
void adjustBranch(unsigned Size, const MCFixup &Fixup, uint64_t &Value,
  ...
  GB::fixups::adjustBranchTarget(Value);
}
...
void GBZ80AsmBackend::adjustFixupValue(const MCFixup &Fixup,
                                     const MCValue &Target,
                                     uint64_t &Value,
                                     MCContext *Ctx) const {
  // The size of the fixup in bits.
  uint64_t Size = GBZ80AsmBackend::getFixupKindInfo(Fixup.getKind()).TargetSize;
  ...
  case GB::fixup_7_pcrel:
    GBadjust::fixup_7_pcrel(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_13_pcrel:
    GBadjust::fixup_13_pcrel(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_call:
    GBadjust::fixup_call(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_ldi:
    GBadjust::ldi::fixup(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_lo8_ldi:
  case GB::fixup_lo8_ldi_pm:
    if (Kind == GB::fixup_lo8_ldi_pm) GBadjust::pm(Value);

    GBadjust::ldi::lo8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_hi8_ldi:
  case GB::fixup_hi8_ldi_pm:
    if (Kind == GB::fixup_hi8_ldi_pm) GBadjust::pm(Value);

    GBadjust::ldi::hi8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_hh8_ldi:
  case GB::fixup_hh8_ldi_pm:
    if (Kind == GB::fixup_hh8_ldi_pm) GBadjust::pm(Value);

    GBadjust::ldi::hh8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_ms8_ldi:
    GBadjust::ldi::ms8(Size, Fixup, Value, Ctx);
    break;

  case GB::fixup_lo8_ldi_neg:
  case GB::fixup_lo8_ldi_pm_neg:
    if (Kind == GB::fixup_lo8_ldi_pm_neg) GBadjust::pm(Value);

    GBadjust::ldi::neg(Value);
    GBadjust::ldi::lo8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_hi8_ldi_neg:
  case GB::fixup_hi8_ldi_pm_neg:
    if (Kind == GB::fixup_hi8_ldi_pm_neg) GBadjust::pm(Value);

    GBadjust::ldi::neg(Value);
    GBadjust::ldi::hi8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_hh8_ldi_neg:
  case GB::fixup_hh8_ldi_pm_neg:
    if (Kind == GB::fixup_hh8_ldi_pm_neg) GBadjust::pm(Value);

    GBadjust::ldi::neg(Value);
    GBadjust::ldi::hh8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_ms8_ldi_neg:
    GBadjust::ldi::neg(Value);
    GBadjust::ldi::ms8(Size, Fixup, Value, Ctx);
    break;
  case GB::fixup_16:
    GBadjust::unsigned_width(16, Value, std::string("port number"), Fixup, Ctx);

    Value &= 0xffff;
    break;
  case GB::fixup_6_adiw:
    GBadjust::fixup_6_adiw(Fixup, Value, Ctx);
    break;

  case GB::fixup_port5:
    GBadjust::fixup_port5(Fixup, Value, Ctx);
    break;

  case GB::fixup_port6:
    GBadjust::fixup_port6(Fixup, Value, Ctx);
    break;
...
MCObjectWriter *GBZ80AsmBackend::createObjectWriter(raw_pwrite_stream &OS) const {
  return createGBZ80ELFObjectWriter(OS,
...
MCFixupKindInfo const &GBZ80AsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  // NOTE: Many GBZ80 fixups work on sets of non-contignous bits. We work around
  // this by saying that the fixup is the size of the entire instruction.
  const static MCFixupKindInfo Infos[GB::NumTargetFixupKinds] = {
...
bool GBZ80AsmBackend::writeNopData(uint64_t Count, MCObjectWriter *OW) const {
...
bool GBZ80AsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                          const MCFixup &Fixup,
                                          const MCValue &Target) {
...
MCAsmBackend *createGBZ80AsmBackend(const Target &T, const MCRegisterInfo &MRI,
  ...
  return new GBZ80AsmBackend(TT.getOS());
}
